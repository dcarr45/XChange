...
public abstract class NettyStreamingService<T> {
                ...
                b.group(group)
                        ...
                        .handler(new ChannelInitializer<SocketChannel>() {
                            ...
                            protected void initChannel(SocketChannel ch) {
                                ...
                                List<ChannelHandler> handlers = new ArrayList<>(3);
        ...
        return Completable.create(completable -> {
            if (webSocketChannel.isOpen()) {
                ...
                for (EventLoopGroup group : eventLoopGroups) {
                    if (!group.shutdownGracefully(0, 10, TimeUnit.SECONDS).awaitUninterruptibly(new Long(30), TimeUnit.SECONDS)) {
                        System.out.println("Failed to shutdown netty event loop!");
                        if (!group.isShutdown()) {
                            group.shutdown();
                            System.out.println("Succeeded in aggressively shutting down event loop");
                        }
                    } else {
                        System.out.println("Netty event loop group shutdown successfully");
                    }
                }
                ...
                System.out.println("Websocket already closed!");

                for (EventLoopGroup group : eventLoopGroups) {
                    if (!group.shutdownGracefully(0, 10, TimeUnit.SECONDS).awaitUninterruptibly(new Long(30), TimeUnit.SECONDS)) {
                        System.out.println("Failed to shutdown netty event loop!");
                    } else {
                        System.out.println("Netty event loop shutdown successful");
                    }
                }
                System.out.println("Websocket was closed but succesfully disconnected");
    ...
    protected void handleChannelMessage(String channel, T message) {
        Subscription channelSub = channels.get(channel);
        if (channelSub != null) {
            ObservableEmitter<T> emitter = channelSub.emitter;
            if (emitter == null) {
            ...
            System.out.println("Not subscribed to any channels!");
        }
    }
...
//                resubscribeChannels();
                LOG.info("Found channel inactive but hopefully handling");
            }
        }
    }
